...
export async function makeDecision(marketData: MarketData, retryCount = 0): Promise<any> {
  ...
  try {
    ...
    const prompt = `You are a conservative trend-following crypto trading agent managing a $10,000 paper portfolio. You receive BTC/USD market data every 5 minutes. Analyze the last 10 hourly OHLC candles to determine trend.
